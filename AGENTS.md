# Dice Chess Bot — Python Starter — AI Agent Guidelines

## Architecture Overview
- **Domain**: Minimal, zero-dependency Python starter template for building Dice Chess bots using the standard library.
- **Contract**: Uses pure wire protocol (HTTP/REST + Webhook HMAC with `urllib`, `hmac`, `hashlib`) without requiring a game engine dependency.
- **Modes**: Standalone polling bot (`bot.py`), local/cloud HTTP webhook handler (`webhook.py`), and registration helper (`register.py`).

## Developer Workflows
- **Setup**: Python 3.9+ (Python 3.12+ recommended).
- **Tests**: `python3 -m unittest discover tests` (runs hermetic test suite).
- **Lint**: `ruff check .`
